...
def get_decp_json(json_files: dict, date_now: str) -> list:
    ...
    for json_file in json_files:
        if json_file["process"] is True:
            ...
            filename = json_file["file_name"]
            # if filename == "decp-2022":
            #     preprocessed = preprocess_json_2022(decp_json)
            #     df_titulaires: pd.DataFrame = pd.json_normalize(preprocessed["titulaires_dict"], record_path="titulaires")
            #     df_titulaires: pl.DataFrame = pl.from_pandas(df_titulaires)
            #
            #     df: pl.DataFrame = pl.json_normalize(preprocessed["marches_dict"]["marches"], strict=False, infer_schema_length=20000)
            #
            #     assert df.shape[0] == df_titulaires.shape[0]
            #     pl.concat([df, df_titulaires], how="horizontal")
            #     pdb.set_trace()
            #     assert "titulaires" in df.columns
